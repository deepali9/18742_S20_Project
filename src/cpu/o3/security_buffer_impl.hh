...
void SecBuf<Impl>::addEntry(InstSeqNum seqNum, Addr addr,
     uint8_t *data, ThreadID tid, const DynInstPtr &inst,
...
bool SecBuf<Impl>::isFillEmpty() {
    ...
    if (!found) {
        DPRINTF(AshishBasic, "SecBuf : Fill empty\n");
        return true;
    }
    else {
        DPRINTF(AshishBasic, "SecBuf : Fill not empty\n");
        return false;
    }
}
...
bool SecBuf<Impl>::getFillEntry(InstSeqNum *secBufseqNum,
    ...
    DynInstPtr *secBufinst, RequestPtr *secBufreq,
    ...
    for (int i = secBufPointer; i < numInstsInSecBuf; i++) {
        if (security_buf[i].fill && security_buf[i].valid) {
            ...
            *secBufinst = security_buf[match_indx].inst;
            *secBufreq = security_buf[match_indx].req;
            *fill_match_indx = match_indx;
...
void SecBuf<Impl>::resetFillPointer() {
    secBufPointer = 0;
    DPRINTF(AshishBasic, "SecBuf : Sec Buf fill pointer "
        "reset %d\n", secBufPointer);
}

/**Increment Security Buffer Pointer**/
//Increment it to be post the match_indx
template <class Impl>
bool SecBuf<Impl>::incrementSecBufPointer(unsigned fill_match_indx) {
    if (fill_match_indx<numInstsInSecBuf-1) {
        secBufPointer = fill_match_indx + 1;
        DPRINTF(AshishBasic, "SecBuf : Incremented secBuf fill"
            " pointer %d\n", secBufPointer);
        return true;
    }
    else {
